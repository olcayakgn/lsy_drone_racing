"""Bishop-frame geometry engine for reference paths, frames, and corridor constraints."""
...
class GeometryEngine:
    """Geometric path, Bishop-frame, and corridor-constraint generation for drone racing.
    ...
    def _insert_obstacle_avoidance_waypoints(
        ...
        # Note: This updates sampled_points in place conceptually (though logic
        # below simplifies this)
        current_points = sampled_points

        for obs in self.obstacles_pos:
            current_points = self._process_single_obstacle_avoidance(
                current_points, clearance_radius, obs
            )
    ...
    def _process_single_obstacle_avoidance(
        ...
        """Identifies segments where the path intersects an obstacle and bends the path outward."""
    ...
    def _get_spline(self, points: NDArray[np.float64]) -> CubicHermiteSpline:
        ...
        # Ideally, if tangents are known, use CubicHermiteSpline. Here we let
        # Scipy estimate tangents.
    ...
    def _generate_parallel_transport_frame(
        self, num_points: int = 3000
    ...
    def _generate_static_corridor(self) -> Dict[str, NDArray[np.float64]]:
        ...
        for i in range(num_pts):
            ...
            for gate_pos in self.gates_pos:
                ...
                if (
                    abs(d_long) <= self.GATE_CONTRACTION_LEN
                    and abs(d_abs) <= self.GATE_CONTRACTION_LEN
                ):
            ...
            for obs in self.obstacles_pos:
                ...
                if abs(w1_obs) < 0.1:  # Ambiguous / Center case
                    ...
                    # Clearly on one side
                    if w1_obs >= 0:
                        # Obs is to the Left (positive n1), so we must be Right
                        # of it -> constrain Upper Bound
        ...
        if np.any(collapsed):
            ...
            print(
                f"[Geometry] Warning: Corridor collapsed at {np.sum(collapsed)} "
                "points. Forced min width."
            )
    ...
    def _add_debug_statement(self, **kwargs: object):
    ...
    def get_closest_s(
        self, pos_query: NDArray[np.float64], s_guess: float = 0.0, window: float = 5.0
    ) -> float:
