...
class SpatialScenarioMPCController(Controller):
    ...
    # Crossing-acceptance margin inside the inner opening [m]. The cost accepts a
    # crossing as a real PASS when |y|,|z| <= clear_cross = GATE_HALF_OPENING -
    # DRONE_RADIUS - GATE_CLEARANCE = 0.20 - 0.05 - 0.01 = 0.14. This must match the
    # env, which counts a pass anywhere in its 0.45 box (|offset| < 0.225); a
    # drone of radius 0.05 physically clears the 0.20 inner edge up to offset 0.15.
    # A larger margin here made MPPI tag high/off-centre crossings (that the env
    # WOULD accept) as W_BAD_CROSS and refuse them, so the drone looped in front of
    # the gate instead of committing. Keep small so every env-valid pass is rewarded.
    GATE_CLEARANCE = 0.01         # crossing-acceptance margin inside the inner opening [m]
    ...
    DIP_APPROACH_DIST = 0.30      # approach distance for wrong-side (dip) gates [m] (shallow = quick momentum dip-through, stays off arena walls)
    DIP_HOOK_WIDTH = 0.60         # lateral swing radius of the dip hook around the gate [m]
    DIP_ALIGN_GUARD = 0.08        # only align/slow once within this signed dist of the -x side [m]
    DIP_PUNCH_TURN_FLOOR = 0.80   # min turn-speed factor for the dip forward crossing (fast re-accel after back-cross)
    DIP_OVERSHOOT_PROBE = 0.55    # how far past a dip gate plane to test for a wall (backside reachability) [m]
    DIP_BACKSIDE_COMMIT = 0.15    # signed depth behind a wall-blocked dip gate to latch "committed" (one-way) [m]
    WALL_MARGIN = 0.06            # keep route waypoints this far inside the arena walls (beyond drone radius) [m]
    ...
    THRUST_TRIM = 1.0            # output thrust multiplier (1.0=sim; >1, e.g. 1.04, offsets real motor/battery deficit)
    ...
    TEMPERATURE = 0.05            # MPPI softmax temperature (lower = greedier)
    ...
    PLANNER_HZ = 25.0             # replanning frequency [Hz]
    ...
    ELITE_TEMPERATURE = 0.018      # softmax temperature for elite blending
    ...
    #  DEBUG / DIAGNOSTICS
    # ==========================================================================
    DEBUG_PRINT_ENABLED = False    # verbose per-step/per-tick diagnostics (episode summary always prints)
    ...
    ROUTE_FULLSCORE_K = 8          # max raw candidates to fully deflect/score (cheap pre-prune above)
    ...
    # ==========================================================================
    W_REF_POS = 0.011             # position tracking error weight
    W_REF_VEL = 0.003             # velocity tracking error weight
    W_TERMINAL_REF = 0.040        # terminal position error weight
    W_PROGRESS = 0.026            # forward progress toward gate reward weight
    W_GATE_DISTANCE_STAGE = 0.0055  # per-step distance-to-gate penalty weight
    W_GATE_DISTANCE_TERMINAL = 0.042  # terminal distance-to-gate penalty weight
    W_GATE_CLOSING = 0.028        # reward for reducing distance to gate per step
    W_GATE_APPROACH_VEL = 0.018   # penalty for retreating from gate within 0.30m (not-yet-crossed)
    W_GATE_ALTITUDE = 0.015       # penalty for being below target gate altitude (climb incentive)
    W_NOT_CROSSED_REACHABLE = 1.8  # penalty for not crossing a reachable gate

    # ==========================================================================
    #  COST WEIGHTS — GATE CROSSING QUALITY
    # ==========================================================================
    W_CROSS_CENTER = 3.5          # soft anchor: mild reward for crossing near center
    W_BAD_CROSS = 38.0            # penalty for crossing outside safe opening (near frame)
    BONUS_GOOD_CROSS = 2.1        # flat bonus for a clean center crossing
    W_CROSS_ALIGN = 2.0           # penalty for crossing at an angle to the gate normal (sin^2)

    # ==========================================================================
    #  COST WEIGHTS — OBSTACLE AVOIDANCE
    # ==========================================================================
    W_POLE_BUFFER = 2.6           # soft buffer zone penalty around poles
    W_POLE_COLLISION = 25.0       # hard collision penalty for poles
    W_POLE_NEAR_EXP = 0.015      # exponential proximity penalty for poles

    # ==========================================================================
    #  COST WEIGHTS — GATE FRAME AVOIDANCE
    # ==========================================================================
    W_FRAME_BUFFER = 9.0          # soft buffer zone penalty around gate frame bars
    W_FRAME_COLLISION = 80.0      # hard collision penalty for gate frame bars
    W_FRAME_SLAB = 35.0           # penalty for being in gate slab outside opening
    W_FUNNEL = 2.8                # funnel centering penalty (guides approach)
    ...
    # ==========================================================================
    W_ALTITUDE = 6.0              # soft altitude boundary penalty
    W_ALTITUDE_HARD = 40.0        # hard altitude violation penalty (floor/ceiling)
    W_SPEED_LIMIT = 0.018         # overspeed penalty weight
    W_INPUT = 0.000025            # control magnitude regularisation
    W_DINPUT = 0.000075           # control rate-of-change regularisation

    # ==========================================================================
    #  COST WEIGHTS — SPATIAL / ROTATIONAL (Bishop frame)
    # ==========================================================================
    W_CORRIDOR = 0.8              # soft corridor boundary penalty
    W_CORRIDOR_HARD = 4.0         # hard corridor violation penalty
    W_SPATIAL_PROGRESS = 0.004    # reward for longitudinal progress along path
    W_CURVATURE_SPEED = 0.002     # penalty for exceeding curvature-adapted speed limit
    W_ATTITUDE_SMOOTH = 0.00015   # angular rate smoothness penalty
    W_ATTITUDE_LIMIT = 0.4        # penalty for roll/pitch beyond safe limit
    ...
    # ==========================================================================
    #  GLOBAL TRAJECTORY PRE-SOLVE (offline MPPI over entire track)
    # ==========================================================================
    PRESOLVE_ENABLED = True            # run global pre-solve in constructor
    PRESOLVE_DT = 0.110                # propagation  time step for pre-solve [s]
    PRESOLVE_V_PLAN = 1.50             # nominal planning speed for horizon estimation [m/s]
    PRESOLVE_K_SAMPLES = 512           # samples per MPPI iteration (no real-time constraint)
    PRESOLVE_ITERATIONS = 80           # MPPI refinement iterations
    PRESOLVE_TEMPERATURE = 0.035       # softmax temperature for pre-solve MPPI
    PRESOLVE_NOISE_RHO = 0.90          # temporal correlation (higher = smoother trajectories)
    PRESOLVE_SIGMA_INIT = np.array([0.30, 0.30, 0.20, 0.10], dtype=np.float64)
    PRESOLVE_SIGMA_MIN = np.array([0.04, 0.04, 0.03, 0.015], dtype=np.float64)
    PRESOLVE_SIGMA_DECAY = 0.97        # sigma decay per iteration (annealing)
    PRESOLVE_HORIZON_PAD = 1.25        # time margin multiplier over estimated flight time
    PRESOLVE_RESAMPLE_SPACING = 0.04   # output trajectory point spacing [m]
    PRESOLVE_MAX_SEGMENT_N = 40        # max horizon steps per gate-to-gate segment
    ...
    def __init__(self, obs: dict[str, "NDArray[np.floating]"], info: dict, config: dict):
        ...
        # Classify gate crossing directions (fixed forward axis + wrong-side "dip" flags)
        self._race_start_pos = np.asarray(obs["pos"], dtype=np.float64)
        self._classify_gate_directions(self._race_start_pos)
        ...
        # Arena XY bounds — clamp route waypoints inside the walls so a dip
        # maneuver next to a boundary (a gate whose back side faces a wall)
        # doesn't place unreachable approach/entry waypoints outside the arena.
        self._arena_xy_low = None
        self._arena_xy_high = None
        try:
            _limits = config.env.track.safety_limits
            _lo = np.asarray(_limits.pos_limit_low, dtype=np.float64)
            _hi = np.asarray(_limits.pos_limit_high, dtype=np.float64)
            if _lo.shape[0] >= 2 and _hi.shape[0] >= 2:
                self._arena_xy_low = _lo[:2].copy()
                self._arena_xy_high = _hi[:2].copy()
        except (AttributeError, KeyError, TypeError, IndexError):
            self._arena_xy_low = None
            self._arena_xy_high = None

        # Now that arena bounds are known, flag dip gates whose backside is
        # wall-blocked (need an explicit swing-around loop, not an overshoot).
        self._classify_loop_gates()
        # One-way dip-staging commit (per target gate). True = drone may punch
        # straight through; only a wall-blocked dip gate that has NOT yet reached
        # its backside sets this False. It latches back to True once behind and is
        # never reset mid-attempt (avoids the loop<->punch oscillation).
        self._dip_committed = True
        ...
        # --- Global trajectory pre-solve (offline MPPI over entire track) ---
        self._global_traj_pos: np.ndarray | None = None   # (M, 3)
        self._global_traj_vel: np.ndarray | None = None   # (M, 3)
        self._global_traj_cum: np.ndarray | None = None   # (M,) arc-length
        self._presolve_global_trajectory(obs)
    ...
    # ------------------------------------------------------------------
    #  Global trajectory pre-solve (offline MPPI over the full track)
    # ------------------------------------------------------------------
    def _presolve_global_trajectory(self, obs: dict) -> None:
        """Run offline segment-by-segment MPPI to produce a globally optimised trajectory.

        Solves one gate-to-gate segment at a time (short, tractable horizon), propagating
        the terminal state between segments. This reliably crosses every gate, unlike a
        single full-track open-loop solve which drifts as attitude errors integrate.
        Called once from ``__init__`` before any ``compute_control`` calls.
        """
        if not self.PRESOLVE_ENABLED or self._n_gates == 0:
            return

        t0 = time.perf_counter()
        # Rolling state, propagated from one segment to the next
        state_pos = np.asarray(obs["pos"], dtype=np.float64)
        state_vel = np.zeros(3, dtype=np.float64)
        state_rpy = np.zeros(3, dtype=np.float64)
        state_drpy = np.zeros(3, dtype=np.float64)

        dt = self.PRESOLVE_DT
        traj_pos_chunks: list[np.ndarray] = []
        traj_vel_chunks: list[np.ndarray] = []

        if self.DEBUG_PRINT_ENABLED:
            print(
                f"[PRE-SOLVE] segment-by-segment over {self._n_gates} gates, "
                f"K={self.PRESOLVE_K_SAMPLES}, iters={self.PRESOLVE_ITERATIONS}"
            )

        for gi in range(self._n_gates):
            gp = self._gate_positions[gi]
            normal = self._get_gate_normal(gi)
            is_dip = bool(self._gate_is_dip[gi])
            app_dist = self.DIP_APPROACH_DIST if is_dip else self.APPROACH_DIST
            approach = gp - app_dist * normal
            exit_pt = gp + self.EXIT_DIST * normal

            # --- Build this segment's route through the gate ---
            seg_route = [state_pos.copy()]
            # Wrong-side (dip) gates: loop laterally around to the -x side first
            hook = self._dip_hook_point(gi, state_pos)
            if hook is not None:
                seg_route.append(hook)
            if float(np.linalg.norm(state_pos - approach)) > 0.20:
                seg_route.append(self._clip_route_point(approach))
            seg_route.append(self._clip_route_point(gp.copy()))
            seg_route.append(self._clip_route_point(exit_pt))
            # Extend slightly toward the next gate for a correct exit heading
            if gi + 1 < self._n_gates:
                gp_next = self._gate_positions[gi + 1]
                hook_next = self._dip_hook_point(gi + 1, exit_pt)
                if hook_next is not None:
                    seg_route.append(hook_next)
                else:
                    n_next = self._get_gate_normal(gi + 1)
                    seg_route.append(self._clip_route_point(gp_next - self.APPROACH_DIST * n_next))
            seg_route = self._clean_route(seg_route)

            # --- Segment horizon from arc length up to the exit point ---
            seg_vecs = seg_route[1:] - seg_route[:-1]
            seg_lens = np.linalg.norm(seg_vecs, axis=1)
            seg_length = float(np.sum(seg_lens))
            est_time = seg_length / max(self.PRESOLVE_V_PLAN, 0.5)
            N_seg = int(math.ceil(est_time * self.PRESOLVE_HORIZON_PAD / dt))
            # Dip gates need a longer loop, so allow extra horizon for them
            n_cap = self.PRESOLVE_MAX_SEGMENT_N + (16 if self._gate_is_dip[gi] else 0)
            N_seg = int(np.clip(N_seg, 12, n_cap))

            best_pos, best_vel, term_state, seg_cost = self._presolve_solve_segment(
                state_pos, state_vel, state_rpy, state_drpy, seg_route, gi, N_seg, dt,
            )

            if best_pos is None:
                print(f"[PRE-SOLVE] gate {gi}: segment failed, aborting pre-solve")
                self._global_traj_pos = None
                return

            # Append segment trajectory (drop the duplicate join point after gate 0)
            start_k = 0 if gi == 0 else 1
            traj_pos_chunks.append(best_pos[start_k:])
            traj_vel_chunks.append(best_vel[start_k:])

            # Propagate terminal state into the next segment
            state_pos, state_vel, state_rpy, state_drpy = term_state

            if self.DEBUG_PRINT_ENABLED:
                print(
                    f"[PRE-SOLVE] gate {gi}: N={N_seg}, len={seg_length:.2f}m, "
                    f"cost={seg_cost:.1f}, term_pos=[{state_pos[0]:+.2f},"
                    f"{state_pos[1]:+.2f},{state_pos[2]:+.2f}]"
                )

        full_pos = np.vstack(traj_pos_chunks)
        full_vel = np.vstack(traj_vel_chunks)

        # --- Resample into a dense polyline for fast runtime lookup ---
        self._global_traj_pos, self._global_traj_vel, self._global_traj_cum = (
            self._presolve_resample(full_pos, full_vel)
        )

        elapsed = time.perf_counter() - t0
        print(
            f"[PRE-SOLVE] done in {elapsed:.2f}s — "
            f"trajectory: {len(self._global_traj_pos)} points, "
            f"arc_length={float(self._global_traj_cum[-1]):.2f}m"
        )

    def _presolve_solve_segment(
        self,
        pos0: np.ndarray,
        vel0: np.ndarray,
        rpy0: np.ndarray,
        drpy0: np.ndarray,
        seg_route: np.ndarray,
        target_gi: int,
        N: int,
        dt: float,
    ) -> tuple[np.ndarray | None, np.ndarray | None, tuple | None, float]:
        """Solve one gate segment with MPPI.

        Returns ``(best_pos, best_vel, terminal_state, best_cost)`` where
        ``terminal_state`` is ``(pos, vel, rpy, drpy)`` of the best trajectory's final
        step, used as the initial state for the next segment.
        """
        # PD warm-start over this segment
        ref_pos, ref_vel = self._presolve_reference(pos0, vel0, seg_route, N, dt)
        mean_u = self._presolve_pd_warmstart(pos0, vel0, rpy0, drpy0, ref_pos, ref_vel, N, dt)

        K = int(self.PRESOLVE_K_SAMPLES)
        rng = np.random.default_rng(42 + target_gi)
        sigma = np.tile(self.PRESOLVE_SIGMA_INIT, (N, 1))
        rho = float(self.PRESOLVE_NOISE_RHO)
        c_rho = math.sqrt(max(1.0 - rho * rho, 0.0))

        lo_clip = np.array([-self.MAX_ROLL_PITCH_CMD, -self.MAX_ROLL_PITCH_CMD,
                            -self.MAX_YAW_CMD, self._thrust_min])
        hi_clip = np.array([self.MAX_ROLL_PITCH_CMD, self.MAX_ROLL_PITCH_CMD,
                            self.MAX_YAW_CMD, self._thrust_max])

        best_pos = best_vel = best_term = None
        best_cost = math.inf

        for _ in range(int(self.PRESOLVE_ITERATIONS)):
            # Antithetic sampling around the mean with AR(1) temporal smoothing
            half_K = max(K // 2, 1)
            eps = rng.standard_normal((half_K, N, 4)) * sigma[None, :, :]
            eps = np.concatenate([eps, -eps], axis=0)[:K]
            smooth = np.empty_like(eps)
            smooth[:, 0, :] = eps[:, 0, :]
            for k in range(1, N):
                smooth[:, k, :] = rho * smooth[:, k - 1, :] + c_rho * eps[:, k, :]

            samples = np.clip(mean_u[None, :, :] + smooth, lo_clip, hi_clip)
            samples[0] = mean_u.copy()  # always evaluate the current mean

            all_pos, all_vel, all_rpy, all_drpy = self._presolve_rollout(
                samples, pos0, vel0, rpy0, drpy0, N, dt,
            )
            costs = self._presolve_segment_cost(
                all_pos, all_vel, samples, ref_pos, ref_vel, N, target_gi,
            )

            # MPPI weighted mean update
            c_min = float(np.min(costs))
            weights = np.exp(-(costs - c_min) / max(float(self.PRESOLVE_TEMPERATURE), 0.01))
            w_sum = float(np.sum(weights))
            weights = weights / w_sum if w_sum > 1e-30 else np.ones(K) / K
            mean_u = np.clip(np.einsum("k,knd->nd", weights, samples), lo_clip, hi_clip)
            sigma = np.maximum(sigma * self.PRESOLVE_SIGMA_DECAY, self.PRESOLVE_SIGMA_MIN)

            b = int(np.argmin(costs))
            if costs[b] < best_cost:
                best_cost = float(costs[b])
                best_pos = all_pos[b].copy()
                best_vel = all_vel[b].copy()
                best_term = (
                    all_pos[b, -1].copy(), all_vel[b, -1].copy(),
                    all_rpy[b, -1].copy(), all_drpy[b, -1].copy(),
                )

        return best_pos, best_vel, best_term, best_cost

    def _presolve_segment_cost(
        self,
        all_pos: np.ndarray,
        all_vel: np.ndarray,
        samples: np.ndarray,
        ...
        """Score one segment's trajectories, focused on crossing the target gate."""
        K = all_pos.shape[0]
        cost = np.zeros(K, dtype=np.float64)

        # --- Reference tracking ---
        for k in range(0, N, 2):
            pos_err = all_pos[:, k + 1] - ref_pos[k + 1]
            vel_err = all_vel[:, k + 1] - ref_vel[k + 1]
            cost += 0.004 * np.sum(pos_err ** 2, axis=1)
            cost += 0.0015 * np.sum(vel_err ** 2, axis=1)

        # --- Target gate crossing (the focus of this segment) ---
        gp = self._gate_positions[target_gi]
        R = self._gate_rotmats[target_gi]
        normal = self._get_gate_normal(target_gi, all_pos[:, 0].mean(axis=0))
        signed = (all_pos - gp[None, None, :]) @ normal  # (K, N+1)
        crossed = np.zeros(K, dtype=bool)
        clear = max(self.GATE_HALF_OPENING - self.GATE_CLEARANCE - self.DRONE_RADIUS, 0.02)
        for k in range(N):
            crossing = (signed[:, k] <= 0.0) & (signed[:, k + 1] > 0.0)
            if not np.any(crossing):
                continue
            idx = np.where(crossing)[0]
            local = (all_pos[idx, k + 1] - gp) @ R
            y_c, z_c = local[:, 1], local[:, 2]
            err = y_c ** 2 + z_c ** 2
            inside = (np.abs(y_c) <= clear) & (np.abs(z_c) <= clear)
            cost[idx[inside]] -= 4.0
            cost[idx[~inside]] += 18.0
            cost[idx] += 2.5 * err
            crossed[idx] |= inside

        # Pull trajectories toward the gate; penalise never cleanly crossing it
        closest = np.min(np.linalg.norm(all_pos - gp[None, None, :], axis=2), axis=1)
        cost += 0.030 * closest
        cost += (~crossed) * 2.0

        # --- Obstacle avoidance ---
        for oi in range(self._n_obstacles):
            op = self._obstacle_positions[oi]
            for k in range(0, N + 1, 3):
                d_xy = np.linalg.norm(all_pos[:, k, :2] - op[:2], axis=1)
                safe = self.OBSTACLE_RADIUS + self.OBSTACLE_BUFFER
                cost += 5.0 * np.maximum(safe - d_xy, 0.0) ** 2
                cost += 0.2 * np.maximum(safe + 0.15 - d_xy, 0.0) ** 2

        # --- Gate frame avoidance (all gates) ---
        for gj in range(self._n_gates):
            gpj = self._gate_positions[gj]
            Rj = self._gate_rotmats[gj]
            for k in range(0, N + 1, 4):
                local = (all_pos[:, k] - gpj) @ Rj
                bar_dist = self._distance_to_gate_bars(local) - self.DRONE_RADIUS
                in_range = np.linalg.norm(all_pos[:, k] - gpj, axis=1) < 0.80
                cost += in_range * 8.0 * np.maximum(-bar_dist, 0.0) ** 2
                cost += in_range * 1.5 * np.maximum(0.06 - bar_dist, 0.0) ** 2

        # --- Altitude bounds ---
        for k in range(0, N + 1, 3):
            z = all_pos[:, k, 2]
            cost += 3.0 * (np.maximum(self.GROUND_CLEARANCE - z, 0.0) ** 2
                              + np.maximum(z - self.CEILING, 0.0) ** 2)

        # --- Speed limit ---
        for k in range(0, N, 3):
            speed = np.linalg.norm(all_vel[:, k + 1], axis=1)
            cost += 0.010 * np.maximum(speed - self.V_MAX, 0.0) ** 2

        # --- Control smoothness + magnitude ---
        for k in range(1, N):
            cost += 0.00004 * np.sum((samples[:, k] - samples[:, k - 1]) ** 2, axis=1)
        cost += 0.000015 * np.sum(np.sum(samples ** 2, axis=2), axis=1)

        # --- Terminal: should be past the target gate ---
        terminal_signed = (all_pos[:, -1] - gp) @ normal
        cost -= 0.025 * np.maximum(terminal_signed, 0.0)
        cost += 0.040 * np.maximum(-terminal_signed, 0.0) ** 2

        return np.nan_to_num(cost, nan=1e12, posinf=1e12, neginf=-1e12)

    def _presolve_reference(
        self,
        pos0: np.ndarray,
        vel0: np.ndarray,
        route: np.ndarray,
        ...
        """Build position/velocity reference for the full-track pre-solve horizon."""
        ref_pos = np.zeros((N + 1, 3), dtype=np.float64)
        ref_vel = np.zeros((N + 1, 3), dtype=np.float64)
        ...
        # Pre-compute turn sharpness at each segment for speed profiling
        ...
        for i in range(len(seg_lens)):
            if i > 0 and seg_lens[i] > 0.02 and seg_lens[i - 1] > 0.02:
                ...
                seg_turn[i] = max(0.0, 1.0 - float(np.dot(d_prev, d_next)))

        avg_speed = self.PRESOLVE_V_PLAN
        for k in range(N + 1):
            t = k * dt
            ...
            if k < N:
                tangent = self._unit(route[seg + 1] - route[seg])
                turn = float(seg_turn[seg])
                v_des = avg_speed * max(0.40, 1.0 - 0.55 * turn)

                # Slow down / align only on the -x approach side of a gate.
                # On a dip gate's +x hooking side we keep speed up (fast dip).
                for gi in range(self._n_gates):
                    gp = self._gate_positions[gi]
                    dist = float(np.linalg.norm(ref_pos[k] - gp))
                    if dist < self.ALIGN_START_DIST:
                        normal = self._get_gate_normal(gi)
                        if float(np.dot(ref_pos[k] - gp, normal)) > self.DIP_ALIGN_GUARD:
                            continue  # still on the wrong (+x) side — don't pre-align/slow
                        blend = 1.0 - dist / max(self.ALIGN_START_DIST, _EPS)
                        tangent = self._unit(
                            (1.0 - blend) * tangent + blend * normal, fallback=tangent,
                        )
                        v_des = min(v_des, self.V_GATE + (avg_speed - self.V_GATE) * (1.0 - blend))
                        break

                ref_vel[k] = tangent * v_des
                ref_vel[k, 2] = float(np.clip(ref_vel[k, 2], -self.VZ_REF_MAX, self.VZ_REF_MAX))

        ref_vel[N] = ref_vel[N - 1]
        return ref_pos, ref_vel

    def _presolve_pd_warmstart(
        self,
        pos0: np.ndarray,
        vel0: np.ndarray,
        rpy0: np.ndarray,
        ...
        """Generate a PD warm-start control sequence over the full pre-solve horizon."""
        seq = np.zeros((N, 4), dtype=np.float64)
        p, v = pos0.copy(), vel0.copy()
        r, dr = rpy0.copy(), drpy0.copy()
        kp = np.array([4.7, 4.7, 4.6], dtype=np.float64)
        kd = np.array([2.8, 2.8, 2.5], dtype=np.float64)

        T_act = self._hover_thrust
        for k in range(N):
            e_p = ref_pos[k + 1] - p
            e_v = ref_vel[k + 1] - v
            a_des = np.array([0.0, 0.0, self._g]) + kp * e_p + kd * e_v
            roll_cmd, pitch_cmd, yaw_cmd, thrust = self._accel_to_attitude_cmd(a_des)
            seq[k] = [roll_cmd, pitch_cmd, yaw_cmd, thrust]

            # Propagate with sub-stepping
            ...
            for _ in range(n_sub):
                if self._has_rotor_lag:
                    T_act += (thrust - T_act) * (sub_dt / self._thrust_time_coef)
                    t_phys = self._acc_coef + self._cmd_f_coef * T_act
                else:
                    t_phys = self._acc_coef + self._cmd_f_coef * thrust
    ...
    def _presolve_rollout(
        ...
        """Forward-integrate K samples over the pre-solve horizon (reuses dynamics logic)."""
        K = samples.shape[0]
        n_sub = self.ROLLOUT_SUB_STEPS
        sub_dt = dt / n_sub
        ...
        c_rpy = self._rpy_coef
        c_drpy = self._rpy_rates_coef
        c_cmd = self._cmd_rpy_coef
        g_z = -self._g
        ...
        if has_rotor_lag:
            t_act = np.full((K, 1), self._hover_thrust, dtype=np.float64)
        ...
        for k in range(N):
            ...
            for _ in range(n_sub):
                ...
                ddrpy_batch = (c_rpy[None, :] * rpy
                               + c_drpy[None, :] * drpy
                               + c_cmd[None, :] * cmd_rpy_k)
    ...
    def _presolve_resample(
        self,
        traj_pos: np.ndarray,
        traj_vel: np.ndarray,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Resample the pre-solved trajectory into uniform arc-length spacing."""
        # Compute arc-length along trajectory
        diffs = traj_pos[1:] - traj_pos[:-1]
        seg_lens = np.linalg.norm(diffs, axis=1)
        cum = np.concatenate([[0.0], np.cumsum(seg_lens)])
        total = float(cum[-1])

        if total < 0.1:
            return traj_pos.copy(), traj_vel.copy(), cum.copy()

        spacing = float(self.PRESOLVE_RESAMPLE_SPACING)
        n_pts = max(10, int(math.ceil(total / spacing)) + 1)
        s_query = np.linspace(0.0, total, n_pts)

        out_pos = np.zeros((n_pts, 3), dtype=np.float64)
        out_vel = np.zeros((n_pts, 3), dtype=np.float64)

        for i, s in enumerate(s_query):
            seg = int(np.clip(int(np.searchsorted(cum, s, side="right") - 1), 0, len(seg_lens) - 1))
            seg_len = max(float(seg_lens[seg]), _EPS)
            alpha = float(np.clip((s - cum[seg]) / seg_len, 0.0, 1.0))
            out_pos[i] = (1.0 - alpha) * traj_pos[seg] + alpha * traj_pos[seg + 1]
            out_vel[i] = (1.0 - alpha) * traj_vel[seg] + alpha * traj_vel[seg + 1]

        out_cum = np.linspace(0.0, total, n_pts)
        return out_pos, out_vel, out_cum
    ...
    def _get_gate_normal(self, gi: int, from_pos: np.ndarray | None = None) -> np.ndarray:
        """Return the gate's FIXED forward axis (gate-local +x expressed in world).

        The environment counts a gate as passed only when the drone crosses the gate
        plane from local -x to local +x. That direction is fixed by the gate
        orientation and must NOT be flipped based on the drone's position — flipping
        causes the controller to reward the wrong crossing direction and makes the
        approach geometry oscillate (the "gate hovering" bug). ``from_pos`` is kept
        for call-site compatibility but is ignored.
        """
        if gi < 0 or gi >= self._n_gates:
            return np.array([1.0, 0.0, 0.0], dtype=np.float64)
        cache = getattr(self, "_gate_forward", None)
        fwd = cache[gi] if cache is not None else self._gate_rotmats[gi][:, 0]
        n = np.linalg.norm(fwd)
        return np.asarray(fwd, dtype=np.float64) / max(float(n), _EPS)

    def _classify_gate_directions(self, start_pos: np.ndarray) -> None:
        """Cache each gate's fixed forward axis and flag wrong-side ("dip") gates.

        A gate is a *dip* gate when the drone reaches it from the +x side: flying
        straight would cross +x -> -x (not counted), so the route must loop around to
        the -x side first. We walk the gate chain (anchored at the race start) and test
        which side of each gate plane the incoming point lies on.
        """
        n = self._n_gates
        self._gate_forward = []
        for gi in range(n):
            f = np.asarray(self._gate_rotmats[gi][:, 0], dtype=np.float64)
            self._gate_forward.append(f / max(float(np.linalg.norm(f)), _EPS))
        self._gate_is_dip = np.zeros(n, dtype=bool)
        prev = np.asarray(start_pos, dtype=np.float64)
        for gi in range(n):
            f = self._gate_forward[gi]
            gp = self._gate_positions[gi]
            self._gate_is_dip[gi] = float(np.dot(prev - gp, f)) > 0.0
            prev = gp + self.EXIT_DIST * f
        # Default: no gate needs an explicit loop. _classify_loop_gates() refines
        # this once arena bounds are known (it needs them to detect a wall-blocked
        # backside). Defined here so the attribute always exists.
        self._gate_needs_loop = np.zeros(n, dtype=bool)
        dips = [int(i) for i in range(n) if self._gate_is_dip[i]]
        if self.DEBUG_PRINT_ENABLED:
            print(f"[GATE-CLASS] dip gates (approached from +x side): {dips}")

    def _classify_loop_gates(self) -> None:
        """Flag dip gates whose backside is blocked by an arena wall (need an explicit loop).

        A normal dip gate is crossed by overshooting to the -x side and swinging back
        with momentum (e.g. custom1 gate 0). That only works if there is room behind
        the gate. When the backside overshoot point falls outside the arena walls
        (e.g. custom1 gate 1, whose -x side is past the +y wall), the drone cannot
        overshoot — it jams and oscillates in front of the plane. Those gates need a
        deliberate swing-around-and-back ("staging") route instead. Requires
        ``_arena_xy_low/high``; if unknown, leaves all flags False (today's behavior).
        """
        n = self._n_gates
        self._gate_needs_loop = np.zeros(n, dtype=bool)
        if self._arena_xy_low is None or self._arena_xy_high is None:
            return
        margin = self.DRONE_RADIUS + self.WALL_MARGIN
        lo = self._arena_xy_low + margin
        hi = self._arena_xy_high - margin
        for gi in range(n):
            if not bool(self._gate_is_dip[gi]):
                continue
            gp = self._gate_positions[gi]
            normal = self._get_gate_normal(gi)
            probe_xy = (gp - self.DIP_OVERSHOOT_PROBE * normal)[:2]
            blocked = bool(np.any(probe_xy < lo) or np.any(probe_xy > hi))
            self._gate_needs_loop[gi] = blocked
        loops = [int(i) for i in range(n) if self._gate_needs_loop[i]]
        # One-line confirmation (always on) that wall-blocked dip detection ran.
        print(f"[GATE-CLASS] wall-blocked dip gates (need loop): {loops}")

    def _dip_hook_point(self, gi: int, from_pos: np.ndarray) -> np.ndarray | None:
        """Lateral swing waypoint that loops a dip gate's route around to the -x side.

        Returns ``None`` for ordinary gates. For dip gates it places a point beside the
        gate (on whichever side the drone is already on, to minimise travel), slightly
        on the +x side so the route curls around the frame instead of cutting through.
        """
        if not bool(self._gate_is_dip[gi]):
            return None
        gp = self._gate_positions[gi]
        f = self._get_gate_normal(gi)
        gate_y = self._unit(self._gate_rotmats[gi][:, 1])
        lat = float(np.dot(np.asarray(from_pos, dtype=np.float64) - gp, gate_y))
        side = 1.0 if lat >= 0.0 else -1.0
        hook = gp + side * self.DIP_HOOK_WIDTH * gate_y + 0.12 * f
        return self._clip_route_point(hook)

    def _dip_staging_active(self) -> bool:
        """True while the target gate is a wall-blocked dip the drone hasn't looped behind yet.

        While active, the route pipeline must keep the swing-around-and-back staging
        loop (not straighten or score it away) so the drone reaches the gate's backside
        before crossing. Goes False permanently for this attempt once the one-way
        ``_dip_committed`` latch flips (drone has reached the backside).
        """
        gi = self._target_gate
        needs_loop = getattr(self, "_gate_needs_loop", None)
        if needs_loop is None or not (0 <= gi < self._n_gates):
            return False
        return bool(needs_loop[gi]) and not getattr(self, "_dip_committed", True)
    ...
    def _clip_route_point(self, point: np.ndarray) -> np.ndarray:
        """Clamp a single route waypoint to safe altitude and arena XY bounds."""
        p = np.asarray(point, dtype=np.float64).copy()
        p[2] = float(np.clip(p[2], self.GROUND_CLEARANCE + 0.03, self.CEILING - 0.05))
        if self._arena_xy_low is not None:
            margin = self.DRONE_RADIUS + self.WALL_MARGIN
            p[:2] = np.clip(p[:2], self._arena_xy_low + margin, self._arena_xy_high - margin)
        return p
    ...
    def _sample_polyline(self, route: np.ndarray, spacing: float | None = None) -> np.ndarray:
        ...
        for i in range(len(route) - 1):
            ...
            n = max(1, int(math.ceil(d / sp)))
            t = (np.arange(1, n + 1, dtype=np.float64) / n)[:, None]
            segs.append((1.0 - t) * a[None, :] + t * b[None, :])
    ...
    def _straighten_near_gates(self, route: np.ndarray) -> np.ndarray:
        ...
        if not (0 <= self._target_gate < self._n_gates):
            return route
        # While staging a wall-blocked dip gate, the route MUST keep its lateral
        # swing-out (hook) to loop behind the gate. Straightening it onto the
        # normal axis would collapse the loop and recreate the front oscillation.
        if self._dip_staging_active():
            return route
    ...
    def _make_gate_tail(self, pos: np.ndarray) -> list[np.ndarray]:
        ...
        gi = self._target_gate
        gp = self._gate_positions[gi]
        normal = self._get_gate_normal(gi)  # fixed gate-local +x (crossing direction)
        is_dip = bool(self._gate_is_dip[gi])
        app_dist = self.DIP_APPROACH_DIST if is_dip else self.APPROACH_DIST
        approach = gp - app_dist * normal
        ...
        tail = []
        if self._dip_staging_active():
            # Wall-blocked dip gate, not yet looped behind: force a swing-around-
            # and-back staging route (open-side hook -> backside approach -> gate
            # -> exit). hook/approach sit at gate altitude, so the reference climbs
            # to gate height while swinging out. Stays in place until the one-way
            # commit latch flips (drone reached the backside); after that the
            # branches below take over for the straight punch-through.
            hook = self._dip_hook_point(gi, pos)
            if hook is not None and float(np.linalg.norm(pos - hook)) > 0.16:
                tail.append(hook)
            if float(np.linalg.norm(pos - approach)) > 0.16:
                tail.append(approach)
        elif is_dip and signed > -0.04:
            # Wrong-side gate and not yet on the -x approach side: loop around first
            hook = self._dip_hook_point(gi, pos)
            if hook is not None and float(np.linalg.norm(pos - hook)) > 0.16:
                tail.append(hook)
            if float(np.linalg.norm(pos - approach)) > 0.16:
            ...
            # Non-dip gates use the full straightening funnel for a clean centred
            # entry. Dip gates skip the deep funnel entry: at FUNNEL_LENGTH the
            # entry point lands far on the -x side (for a wall-adjacent gate like
            # custom1's gate 1 that is behind the arena wall and unreachable). The
            # shallow approach above is the -x staging point instead.
            if not is_dip and signed < -0.20 and float(np.linalg.norm(pos - entry)) > 0.20:
                tail.append(entry)
        ...
        if next_gate < self._n_gates:
            ...
            hook_next = self._dip_hook_point(next_gate, exit_pt)
            if hook_next is not None:
                tail.append(hook_next)
            else:
                normal_next = self._get_gate_normal(next_gate)
    ...
    def _score_route(
        ...
        if 0 <= self._target_gate < self._n_gates:
            ...
            if self._dip_staging_active():
                # Looping a wall-blocked dip gate: aim the first move at the
                # swing-out hook (the open-side staging waypoint), not the gate,
                # and skip the cross-track straightness penalty — the lateral
                # swing IS the plan here, so punishing lateral offset would fight
                # the loop and recreate the front oscillation.
                aim_pt = self._dip_hook_point(self._target_gate, pos)
                if aim_pt is None:
                    aim_pt = gp - self.DIP_APPROACH_DIST * self._get_gate_normal(self._target_gate)
                aim_dir = self._unit(aim_pt - pos, fallback=first_dir)
                align = float(np.dot(first_dir, aim_dir))
        ...
        for gi in range(self._n_gates):
            ...
            # First plane crossing (sign change) along the route, if any
            cross_js = np.flatnonzero(signed[:-1] * signed[1:] < 0.0)
            if cross_js.size > 0:
                cross_pt = sampled[int(cross_js[0])]
        ...
        if 0 <= self._target_gate < self._n_gates and not self._dip_staging_active():
            gp = self._gate_positions[self._target_gate]
    ...
    def _build_route_candidates(
        ...
        # Wall-blocked dip gate, not yet looped behind: emit ONLY loop candidates.
        # Straight-to-gate routes are shorter, so the length term would make them
        # win and the drone would cut the corner in front of the plane instead of
        # swinging around to the backside. A few hook-width variants give MPPI
        # smooth choices; the small candidate count also makes staging cheaper
        # (fewer routes to score = smoother sim while looping).
        if self._dip_staging_active():
            gate_y = self._unit(R[:, 1])
            side = 1.0 if float(np.dot(pos - gp, gate_y)) >= 0.0 else -1.0
            back = gp - self.DIP_APPROACH_DIST * normal
            for wmul in (0.85, 1.0, 1.20):
                hook = self._clip_route_point(
                    gp + side * wmul * self.DIP_HOOK_WIDTH * gate_y + 0.12 * normal
                )
                add_route([pos.copy(), hook, back, gp.copy(), exit_pt])
            if not candidates:
                add_route([pos.copy()] + tail)
            return self._finalize_route_candidates(candidates, pos, vel)
    ...
    def _finalize_route_candidates(
        self,
        candidates: list[np.ndarray],
        pos: np.ndarray,
        vel: np.ndarray,
    ) -> tuple[list[np.ndarray], list[float]]:
        """Pre-prune, deflect/straighten, then full-score raw candidates; return top K."""
        # Cheap geometric pre-prune: when many raw candidates exist, keep only the
        # most promising (length + gate alignment + obstacle/heading proxy) before
        # the expensive deflect / straighten / full-score pipeline runs on each.
        if len(candidates) > int(self.ROUTE_FULLSCORE_K):
            candidates = self._preselect_candidates_cheap(candidates, pos, vel)
    ...
    def _preselect_candidates_cheap(
        self,
        candidates: list[np.ndarray],
        pos: np.ndarray,
        vel: np.ndarray,
    ) -> list[np.ndarray]:
        """Rank raw routes by a cheap proxy of _score_route and keep the top K.

        Uses only route vertices (no polyline resampling, no gate-frame distance
        passes) so it is ~100x cheaper than the full score, while preserving the
        dominant ranking terms (length, gate alignment, obstacle/heading penalties).
        """
        ...
        if target_valid:
            gp = self._gate_positions[self._target_gate]
            if self._dip_staging_active():
                # Looping a wall-blocked dip gate: rank by heading toward the
                # swing-out hook (then backside), matching _score_route's staging
                # aim, so the cheap preselect doesn't discard the looping routes.
                aim_pt = self._dip_hook_point(self._target_gate, pos)
                if aim_pt is None:
                    aim_pt = gp - self.DIP_APPROACH_DIST * self._get_gate_normal(self._target_gate)
                gate_dir = self._unit(aim_pt - pos)
            else:
                gate_dir = self._unit(gp - pos)
        obs_xy = (
            self._obstacle_positions[:, :2]
            if self._obstacle_positions.size > 0 else None
        )
        soft = self.OBSTACLE_RADIUS + self.OBSTACLE_BUFFER + 0.12
        speed_xy = float(np.linalg.norm(vel[:2]))
        vel_dir_xy = (
            self._unit(np.array([vel[0], vel[1], 0.0]))[:2] if speed_xy > 0.35 else None
        )

        scored: list[tuple[float, np.ndarray]] = []
        for r in candidates:
            s = float(np.sum(np.linalg.norm(r[1:] - r[:-1], axis=1)))  # length
            first_dir = self._unit(r[1] - r[0])
            if gate_dir is not None:
                s += 7.0 * max(0.0, 1.0 - float(np.dot(first_dir, gate_dir)))
            if vel_dir_xy is not None:
                first_xy = self._unit(np.array([first_dir[0], first_dir[1], 0.0]))[:2]
                heading_align = float(np.dot(vel_dir_xy, first_xy))
                s += 18.0 * speed_xy * max(0.0, 0.5 - heading_align) ** 2
            if obs_xy is not None:
                d_min = float(np.min(
                    np.linalg.norm(r[:, None, :2] - obs_xy[None, :, :2], axis=2)
                ))
                if d_min < soft:
                    s += 180.0 * ((soft - d_min) / soft) ** 2
            scored.append((s, r))
        scored.sort(key=lambda item: item[0])
        return [r for _, r in scored[: int(self.ROUTE_FULLSCORE_K)]]

    # ------------------------------------------------------------------
    #  Global trajectory interpolation (for pre-solved reference)
    # ------------------------------------------------------------------
    def _interpolate_global_trajectory(
        self,
        pos: np.ndarray,
        vel: np.ndarray,
    ) -> tuple[np.ndarray, np.ndarray]:
        """Interpolate the pre-solved global trajectory into MPC_N+1 reference points.

        Projects the drone's current position onto the global trajectory (nearest point),
        then walks forward along it at the pre-solved speed profile to fill the MPC horizon.
        """
        g_pos = self._global_traj_pos   # (M, 3)
        g_vel = self._global_traj_vel   # (M, 3)
        g_cum = self._global_traj_cum   # (M,)
        M = len(g_pos)
        total_s = float(g_cum[-1])

        ref_pos = np.zeros((self.MPC_N + 1, 3), dtype=np.float64)
        ref_vel = np.zeros((self.MPC_N + 1, 3), dtype=np.float64)

        # --- Find nearest point on global trajectory ---
        dists = np.linalg.norm(g_pos - pos[None, :], axis=1)
        nearest_idx = int(np.argmin(dists))

        # Refine: among nearby points, prefer ones ahead of the drone (in velocity direction)
        speed = float(np.linalg.norm(vel))
        if speed > 0.2:
            vel_dir = vel / speed
            search_range = max(5, M // 20)
            lo = max(0, nearest_idx - search_range)
            hi = min(M, nearest_idx + search_range)
            candidates = np.arange(lo, hi)
            d = dists[candidates]
            # Slight bias toward points in the velocity direction
            fwd = (g_pos[candidates] - pos[None, :]) @ vel_dir
            score = d - 0.15 * np.maximum(fwd, 0.0)
            nearest_idx = int(candidates[np.argmin(score)])

        s_start = float(g_cum[nearest_idx])

        # --- Walk forward along the trajectory at a blend of drone speed and traj speed ---
        drone_speed = max(float(np.linalg.norm(vel)), 0.2)
        traj_speed = max(float(np.linalg.norm(g_vel[nearest_idx])), 0.3)
        # Blend: use drone speed when slow (takeoff), trajectory speed when up to speed
        speed_blend = min(drone_speed / max(traj_speed, 0.3), 1.0)
        walk_speed = drone_speed + speed_blend * (traj_speed - drone_speed)
        walk_speed = max(walk_speed, 0.25)  # minimum to always show some lookahead

        for k in range(self.MPC_N + 1):
            t = k * self.MPC_DT
            s_query = min(s_start + walk_speed * t, total_s)

            # Binary search for the segment containing s_query
            seg = int(np.clip(
                int(np.searchsorted(g_cum, s_query, side="right") - 1), 0, M - 2,
            ))
            seg_len = max(float(g_cum[seg + 1] - g_cum[seg]), _EPS)
            alpha = float(np.clip((s_query - g_cum[seg]) / seg_len, 0.0, 1.0))

            ref_pos[k] = (1.0 - alpha) * g_pos[seg] + alpha * g_pos[seg + 1]
            ref_vel[k] = (1.0 - alpha) * g_vel[seg] + alpha * g_vel[seg + 1]

            ref_pos[k, 2] = float(np.clip(
                ref_pos[k, 2], self.GROUND_CLEARANCE, self.CEILING,
            ))
            if k < self.MPC_N:
                ref_vel[k, 2] = float(np.clip(
                    ref_vel[k, 2], -self.VZ_REF_MAX, self.VZ_REF_MAX,
                ))

        # First reference point should be close to current position
        ref_pos[0] = pos.copy()
        ref_pos[0, 2] = float(np.clip(ref_pos[0, 2], self.GROUND_CLEARANCE, self.CEILING))
    ...
    def _generate_references(
        ...
        # --- Use pre-solved global trajectory if available (only when no specific route given) ---
        if (self._global_traj_pos is not None
                and self._target_gate >= 0
                and route_points is None):
            ref_pos, ref_vel = self._interpolate_global_trajectory(pos, vel)
        ...
        gp = self._gate_positions[self._target_gate]
        normal = self._get_gate_normal(self._target_gate, pos)
        target_is_dip = bool(self._gate_is_dip[self._target_gate])
        ...
        for k in range(self.MPC_N + 1):
            ...
            if k < self.MPC_N:
                ...
                signed_k = float(np.dot(ref_pos[k] - gp, normal))
                ...
                # Dip punch: once a wrong-side (dip) gate has been back-crossed and the
                # drone is on the -x side (signed < 0), accelerate hard in the +normal
                # crossing direction instead of decaying to V_GATE. This minimises dwell
                # time on the reversal. The +x approach and the hook turnaround (signed > 0)
                # are left untouched, so the turnaround itself stays slow.
                dip_punch = target_is_dip and signed_k < 0.0

                if dist_to_gate < self.ALIGN_START_DIST and signed_k <= self.DIP_ALIGN_GUARD:
                    ...
                    v_des = self.V_CRUISE
                if dip_punch:
                    turn_speed_factor = max(turn_speed_factor, self.DIP_PUNCH_TURN_FLOOR)
    ...
    def _build_jax_fn(self) -> jax.stages.Wrapped:
        ...
        W_CAL = float(self.W_CROSS_ALIGN)
        ...
        def _single_sample(
            ...
            def scan_step(carry: tuple, xs: tuple) -> tuple[tuple, jax.Array]:
                ...
                # Incidence-angle penalty: discourage slicing through the opening at
                # an angle to the gate normal (sin^2 of the velocity-vs-normal angle).
                # ~0 on a clean perpendicular pass, ->1 for a sideways graze. This makes
                # the planner line up earlier and stops too-sharp side approaches that
                # clip the frame edge.
                v_align = jnp.dot(vel, tgt_n)
                v_sq = jnp.sum(vel * vel)
                sin2 = jnp.clip(1.0 - jnp.maximum(v_align, 0.0) ** 2 / jnp.maximum(v_sq, EPS), 0.0, 1.0)
                cost = cost + cf * W_CAL * sin2
                ba = (
            ...
            cost_f = cost_f - gc_f.astype(jnp.float32) * 0.250 * tv
            cost_f = cost_f + (
    ...
    def _score_rollouts(
        ...
        # --- NumPy fallback (used for batched route-ranking calls) ---
        # ref_pos/ref_vel may be either (N+1,3) shared or (K,N+1,3) per-sample.
        ref_batched = ref_pos.ndim == 3
        ...
        for k in range(N):
            ...
            # --- Reference tracking ---
            e_p = pos - (ref_pos[:, k + 1] if ref_batched else ref_pos[k + 1])
            e_v = vel - (ref_vel[:, k + 1] if ref_batched else ref_vel[k + 1])
            cost += stage_w * (
        ...
        terminal_error = terminal_pos - (ref_pos[:, -1] if ref_batched else ref_pos[-1])
    ...
    def _candidate_progress_cost(
        ...
        select_cost += 9.0 * np.maximum(self.SELECTION_MARGIN - min_margin[indices], 0.0) ** 2
        select_cost -= 0.140 * np.maximum(min_margin[indices] - self.SELECTION_MARGIN, 0.0)
        if cross_yz is not None:
            yz = cross_yz[indices]
            finite = np.isfinite(yz)
            select_cost[finite] += 0.700 * (yz[finite] / max(self.ROBUST_CROSS_RADIUS, _EPS)) ** 2
        if 0 <= self._target_gate < self._n_gates:
            gp = self._gate_positions[self._target_gate]
            ...
            select_cost -= 0.620 * progress
            select_cost += 0.115 * d_final
        return select_cost
    ...
    def _rank_routes_by_rollout(
        ...
        # Generate per-route references + PD warm-start sequences, then score ALL
        # routes in a SINGLE batched rollout (per-sample references) instead of one
        # slow K=1 NumPy call per route. Mathematically identical to the per-route
        # loop (no cross-sample coupling in the cost), just far fewer Python passes.
        seqs, refs_pos, refs_vel = [], [], []
        for route in routes:
            r_pos, r_vel = self._generate_references(pos, vel, route)
            seqs.append(self._make_pd_sequence(pos, vel, rpy, drpy, r_pos, r_vel))
            refs_pos.append(r_pos)
            refs_vel.append(r_vel)
        samples = np.stack(seqs, axis=0)        # (R, N, 4)
        ref_pos_b = np.stack(refs_pos, axis=0)  # (R, N+1, 3)
        ref_vel_b = np.stack(refs_vel, axis=0)  # (R, N+1, 3)
        c, margin, traj, crossed, bad = self._score_rollouts(
            samples, pos, vel, rpy, drpy, ref_pos_b, ref_vel_b,
        )

        records = []
        for idx, route in enumerate(routes):
            c0, m0 = float(c[idx]), float(margin[idx])
            crossed0, bad0 = bool(crossed[idx]), bool(bad[idx])
            geom = float(route_costs[idx]) if idx < len(route_costs) else 0.0

            select_score = c0 + 0.00004 * geom
            ...
            if bad0:
                select_score += 20.0
            if m0 < 0.0:
                select_score += 12.0 * (0.02 - m0) ** 2 + 2.5

            bucket = 0 if (crossed0 and not bad0 and m0 > 0.0) else (1 if m0 > 0.0 else 2)
            records.append((
                bucket, select_score, idx, route, geom, seqs[idx], refs_pos[idx], refs_vel[idx],
                c0, m0, crossed0, bad0, traj[idx].astype(np.float64),
            ))
    ...
    def _finish_action(
        ...
        # Real-world thrust trim: compensate motor/battery thrust deficit (1.0 = sim, no-op).
        thrust_cmd *= self.THRUST_TRIM
    ...
    def compute_control(
        ...
        if "target_gate" in obs:
            ...
            if obs_target_gate != self._target_gate:
                ...
                self._last_best_traj = None
                self._last_route_anchor_traj = None
                # Seed the one-way dip-staging commit for the new target. A
                # wall-blocked dip gate starts UNcommitted (must loop to its
                # backside first) unless the drone is already behind its plane.
                # Every other gate is committed immediately = normal behavior.
                committed = True
                if 0 <= obs_target_gate < self._n_gates and bool(self._gate_needs_loop[obs_target_gate]):
                    _dn = self._get_gate_normal(obs_target_gate)
                    _ds = float(np.dot(pos - self._gate_positions[obs_target_gate], _dn))
                    committed = _ds < 0.0
                    print(
                        f"[DIP-STAGE] gate{obs_target_gate}: "
                        f"{'committed (already behind)' if committed else 'STAGING ON -> loop to backside'} "
                        f"(signed={_ds:+.2f})"
                    )
                self._dip_committed = committed
            self._target_gate = obs_target_gate
            if self._target_gate < 0:
                self._finished = True
                return np.array([0.0, 0.0, 0.0, self._hover_thrust], dtype=np.float32)

        # --- One-way dip-staging latch (every tick) ---
        # Once a wall-blocked dip gate's backside is reached, commit to punching
        # forward and NEVER un-commit for this attempt. One-way is the key
        # difference from the earlier oscillating version: without a reset, the
        # drone cannot flip between loop and punch in front of the plane.
        _gi = self._target_gate
        if (
            not self._dip_committed
            and 0 <= _gi < self._n_gates
            and bool(self._gate_needs_loop[_gi])
        ):
            _ln = self._get_gate_normal(_gi)
            _ls = float(np.dot(pos - self._gate_positions[_gi], _ln))
            if _ls <= -self.DIP_BACKSIDE_COMMIT:
                self._dip_committed = True
        ...
        # Inject global trajectory warm-start as additional candidate
        if self._global_traj_pos is not None and self._target_gate >= 0:
            gt_ref_pos, gt_ref_vel = self._interpolate_global_trajectory(pos, vel)
            self._apply_launch_altitude_ramp(gt_ref_pos, gt_ref_vel)
            gt_seq = self._make_pd_sequence(pos, vel, rpy, drpy, gt_ref_pos, gt_ref_vel)
            route_seqs.append(gt_seq)
            # Use global trajectory reference for scoring (blended with route ref)
            ref_pos = gt_ref_pos
            ref_vel = gt_ref_vel
        ...
        # Roll out the mean control sequence to get the mean trajectory for visualisation
        mean_pos, _, _, _ = self._rollout_rpy_thrust(
            self._mean_u[None, :, :], pos, vel, rpy, drpy,
        )
        self._last_mean_traj = mean_pos[0].astype(np.float64)  # (N+1, 3)
    ...
    def step_callback(
        ...
        if any_gate_changed or any_obs_changed:
            if any_gate_changed:
                # Gate poses refined by sensors — recompute fixed forwards / dip flags
                self._classify_gate_directions(self._race_start_pos)
            self._rebuild_geometry()
    ...
    def render_callback(self, sim: "Sim"):
        """Visualise the best MPPI sample and the route-anchor reference path."""
        from crazyflow.sim.visualize import draw_line, draw_points

        # Green: best selected MPPI sample trajectory
        ...
        # Blue: route-anchor reference trajectory (same as SpatialScenarioMPC)
